...
function inputCheck(){
	...
	var spacePost = str.indexOf(' ');
	var commaPost = str.indexOf(',');
	var eMailSize = str.length;
	
	
	if(atPos > 1 && atPos == atLastPos && dotPos > 3 && spacePost == -1 && commaPost == -1 && atPos + 1 < dotPos && dotPos + 1 < eMailSize);
	else{
		alert("E_mail 주소 형식이 잘못되었습니다. 다시 입력.");
		document.regForm.email.focus();
		return;
	}
